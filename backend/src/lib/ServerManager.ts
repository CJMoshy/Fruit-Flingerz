export default class ServerManager {
  ...
  removeUserFromSpritesList(uid: UserID) {
    const index = this.spritesList.findIndex((user) => user.user_id === uid);
    if (index === -1) {
      console.log(
  ...
  getConnectedUserIndexBySocketID(socketID: string) {
    return this.connectedUsers.findIndex((user) => user.socket.id === socketID);
  }
  ...
  updateSpriteInList(usr: PlayerMetadata) {
    const index = this.spritesList.findIndex((e) => e.user_id === usr.user_id);
    if (index === -1) return;
